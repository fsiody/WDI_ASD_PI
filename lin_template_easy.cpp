...
void euler(int u) {
    while (!graph[u].empty())
    {
		int v = graph[u].back();// take first neighbour of u // iterate over neighbours of v   // you can use list<int>::iterator it = graph[v].begin()
        // which makes "it" iterator (sth like pointer) pointing on first element of graph[v] list  // iterator can be moved to next position like this: it++
        for (list<int>::iterator it = graph[v].begin(); it != graph.end(); it++) // and if you move it after last element of list it will be equal to graph[v].end()
        {
            if (it==u) // if you find u remove it from list
            { // you can access element pointed by iterator just like with pointer
				graph->erase(it); // use erase method
            }
        }
		euler(v);  // recursively enter v
    }
	line->push_back(u);  // add city to line
