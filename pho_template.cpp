...
int hashfunc(string name) {
	int waga = 9; int suma = 0;
	for (int i = 0; name[i] != 0; i++) {
		suma += suma * waga + name[i];
	}
	return suma;
}
...
string get_number(Slot** hashtable, int n, string name) {
    ...
    int hash = hashfunc(name) % n;
	int np = 1;
    while (!(hashtable[hash]->state==OCCUPIED && hashtable[hash]==name) || hashtable[hash]->state!=EMPTY) {
		hash = (hashfunc(name) + 77 * np) % n;
		np++;
    }
